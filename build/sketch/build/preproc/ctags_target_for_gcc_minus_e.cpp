# 1 "c:\\Dev\\2017\\RebelMakers\\Arduino\\DalekbotSensors\\main.ino"
# 1 "c:\\Dev\\2017\\RebelMakers\\Arduino\\DalekbotSensors\\main.ino"

# 3 "c:\\Dev\\2017\\RebelMakers\\Arduino\\DalekbotSensors\\main.ino" 2
# 4 "c:\\Dev\\2017\\RebelMakers\\Arduino\\DalekbotSensors\\main.ino" 2
# 5 "c:\\Dev\\2017\\RebelMakers\\Arduino\\DalekbotSensors\\main.ino" 2
# 6 "c:\\Dev\\2017\\RebelMakers\\Arduino\\DalekbotSensors\\main.ino" 2
# 7 "c:\\Dev\\2017\\RebelMakers\\Arduino\\DalekbotSensors\\main.ino" 2
...
NewPing sonar1(2, 5 /* you  can use a single pin for trigger/echo if the board is a 5v one.*/, 200 /* max distance in cm. same for all*/);
NewPing sonar2(7, 6, 200 /* max distance in cm. same for all*/);
NewPing sonar3(8, 3, 200 /* max distance in cm. same for all*/);
NewPing sonar4(9, 4, 200 /* max distance in cm. same for all*/);
...
extern "C" void __vector_17 /* SPI Serial Transfer Complete */ (void) __attribute__ ((signal,used, externally_visible)) ; void __vector_17 /* SPI Serial Transfer Complete */ (void)
{
  ...
  slaveValueReceived = (*(volatile uint8_t *)((0x2E) + 0x20));
  switch((*(volatile uint8_t *)((0x2E) + 0x20))){
    case 0:
    //Serial.print("Current device: 0");  
    currentDevice = (*(volatile uint8_t *)((0x2E) + 0x20));
   ...
   // Serial.print("Current device: 5");  
    ...
    case 200:
    (*(volatile uint8_t *)((0x2E) + 0x20)) = sensorValues[currentDevice] >> 8;

    break;

    // send back the low bits
    case 201:
    (*(volatile uint8_t *)((0x2E) + 0x20)) = sensorValues[currentDevice] & 0xff;
    break;
    ...
    default:
    Serial.print("    err: ");
    Serial.print((*(volatile uint8_t *)((0x2E) + 0x20)));
  }
}
